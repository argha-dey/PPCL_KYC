...
class RegistrationActivity : Activity() {
    ...
    private fun initViews() {
        ...
        val tv_address1 = findViewById<EditText>(R.id.tv_address1) as EditText
        val tv_address2 = findViewById<EditText>(R.id.tv_address2) as EditText
        ...
        val rg_gender = findViewById<RadioGroup>(R.id.rg_gender) as RadioGroup
        val rb_male = findViewById<RadioButton>(R.id.rb_male) as RadioButton
        val rb_female = findViewById<RadioButton>(R.id.rb_female) as RadioButton


        tv_confirm_password.setOnEditorActionListener { textView, actionId, keyEvent ->
            if (actionId == EditorInfo.IME_ACTION_DONE) {
                    if(isOnline(this@RegistrationActivity) ){
                        registrationApiCall()
                    }
                    else{
        ...
        val  rl_submit_button =  findViewById<RelativeLayout>(R.id.rl_submit_button) as RelativeLayout
        rl_submit_button.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                if(isOnline(this@RegistrationActivity) ){
                    registrationApiCall()
                }
    ...
    private fun registrationApiCall() {
        ...
        else if (tv_address1.text.toString().trim().isEmpty()) {
            tv_address1.error = "Required"
            Toast.makeText(applicationContext, "Address Required ", Toast.LENGTH_SHORT).show()
        }
        else if (tv_address2.text.toString().trim().isEmpty()) {
            tv_address2.error = "Required"
            Toast.makeText(applicationContext, "Landmark Required ", Toast.LENGTH_SHORT).show()
        }
    ...
    /*    else if (Utils.isValidPassword(tv_password.text.toString().trim())) {
            ...
            requestBody.put("USR_GENDER",gender)
            requestBody.put("state", statename)
            requestBody.put("city",cityname)
            requestBody.put("address",tv_address1.text.toString()+", "+tv_address2.text.toString())
